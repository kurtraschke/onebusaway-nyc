...
import org.onebusaway.nyc.presentation.model.EExactMatchType;
 ...
 * interface specific. We need to create interface-specific models to pass into
...
public class SearchServiceImpl implements SearchService {
    ...
    public SearchResultCollection getSearchResults(String query, SearchResultFactory resultFactory) {
        ...
        if (results.getExactMatchType() == EExactMatchType.STOP) {
            tryAsExactStop(results, normalizedQuery, resultFactory);
        }
        
        if (results.getExactMatchType() == EExactMatchType.ROUTE) {
            tryAsExactRoute(results, normalizedQuery, resultFactory);
        }
    ...
    private String normalizeQuery(SearchResultCollection results, String q) {
        ...
        if (q.startsWith("route:")) {
            results.setExactMatchType(EExactMatchType.ROUTE);
            q = q.replaceFirst("^route:", "");
        } else if (q.startsWith("stop:")) {
            results.setExactMatchType(EExactMatchType.STOP);
            q = q.replaceFirst("^stop:", "");
        } else {
            results.setExactMatchType(EExactMatchType.NONE);
        }
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (_routeShortNameToRouteBeanMap.containsKey(token)) {
                ...
                if ((lastItem != null && !_routeShortNameToRouteBeanMap.containsKey(lastItem))
                    ...
                    results.addRouteFilters(ImmutableSet.copyOf(_routeShortNameToRouteBeanMap.get(token)));
    ...
    private void tryAsExactStop(SearchResultCollection results, String agencyAndId, SearchResultFactory resultFactory) {
        ...
        try {
            ...
            if (potentialStop != null) {
                results.addMatch(resultFactory.getStopResult(potentialStop, filterRouteFilters(results.getRouteFilter())));
            }
    ...
    private void tryAsExactRoute(SearchResultCollection results, String agencyAndId, SearchResultFactory resultFactory) {

        try {
            RouteBean potentialRoute = _nycTransitDataService.getRouteForId(agencyAndId);
            if (potentialRoute != null) {
                results.addMatch(resultFactory.getRouteResult(potentialRoute));
            }
        } catch (ServiceException e) {
            //no-op
        }
    ...
    private void tryAsRoute(SearchResultCollection results, String routeQuery, SearchResultFactory resultFactory) {
        ...
        List<RouteBean> routes = _nycTransitDataService.getRoutes(makeSearchQuery(routeQuery)).getRoutes();

        if (routes.size() == 1) {
            RouteBean routeBean = routes.get(0);
            results.addMatch(resultFactory.getRouteResult(routeBean));
        } else {
            for (RouteBean route : routes) {
                results.addSuggestion(resultFactory.getRouteResult(route));
            }
        }
    }
    ...
    private void tryAsStop(SearchResultCollection results, String stopQuery, SearchResultFactory resultFactory) {
        ...
        List<StopBean> matches = _nycTransitDataService.getStops(makeSearchQuery(stopQuery)).getStops();

        if (matches.size() == 1) {
            results.addMatch(resultFactory.getStopResult(matches.get(0), results.getRouteFilter()));
        } else {
            //If there are multiple stop matches and a route filter,
            //start by eliminating stops which do not match the route filter.
            //This may leave us with an exact stop match.
            
            List<StopBean> candidates = new ArrayList<StopBean>();

            for (StopBean match : matches) {
                if (results.getRouteFilter().size() > 0) {
                    Set<RouteBean> matchRoutes = ImmutableSet.copyOf(match.getRoutes());

                    if (!matchRoutes.containsAll(results.getRouteFilter())) {
                        continue;
                    }
                }
            ...
            if (candidates.size() == 1) {                
                Set<RouteBean> routeFilters = Sets.intersection(results.getRouteFilter(), ImmutableSet.copyOf(candidates.get(0).getRoutes()));
                results.addMatch(resultFactory.getStopResult(candidates.get(0), filterRouteFilters(routeFilters)));
            } else {
                for (StopBean stop : candidates) {
                    Set<RouteBean> routeFilters = Sets.intersection(results.getRouteFilter(), ImmutableSet.copyOf(stop.getRoutes()));
                    results.addSuggestion(resultFactory.getStopResult(stop, filterRouteFilters(routeFilters)));
                }
            }
        }
    }
    ...
    private void tryAsGeocode(SearchResultCollection results, String query, SearchResultFactory resultFactory) {
        ...
        for (NycGeocoderResult result : geocoderResults) {
            if (geocoderResults.size() == 1) {
                results.addMatch(resultFactory.getGeocoderResult(result, filterRouteFilters(results.getRouteFilter())));
            } else {
                results.addSuggestion(resultFactory.getGeocoderResult(result, filterRouteFilters(results.getRouteFilter())));
            }
        }
    }
    ...
    private Set<RouteBean> filterRouteFilters(Collection<RouteBean> routeFilters) {
        Set<RouteBean> _routeFilters = new HashSet<RouteBean>(routeFilters);
